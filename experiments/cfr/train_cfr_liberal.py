...
LOG_EVERY = 1000
EVAL_EVERY = 10000      # More frequent eval
...
class CFRPlusAgent:
    """
    CFR+ agent - regrets are floored at 0 after each update.
    
    This leads to faster convergence than vanilla CFR.
    """
    ...
    def get_strategy(self, infoset_key, legal_actions):
        """Get current strategy via regret matching (CFR+ uses non-negative regrets)."""
        regrets = self.regret_sums[infoset_key]
        
        # In CFR+, regrets are already non-negative
...
def cfr_liberal_iteration(env, cfr_agent, traverser_idx):
    ...
    This trains liberals to play well against random fascist opponents.
    """
...
def main():
    print("=" * 70)
    print("CFR+ LIBERAL TRAINING (vs Random Fascists)")
    print("=" * 70)
    ...
    config = {
        "algorithm": "cfr_plus_liberal_only",
    ...
    if use_wandb:
        wandb.init(
            ...
            entity=WANDB_ENTITY,
            config=config,
            name=f"cfr_liberal_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        )
    ...
    # Create agent (card selection is hard-coded, not learned)
    cfr_agent = CFRPlusAgent()
    ...
    for iteration in range(1, NUM_ITERATIONS + 1):
        ...
        # (only liberals will actually train)
        ...
        if iteration % EVAL_EVERY == 0:
            ...
            results_file = results_dir / f"cfr_liberal_{iteration}_{timestamp}.json"
            
            results_with_meta = {
                "agent_type": "cfr_plus_liberal",
        ...
        if iteration % CHECKPOINT_EVERY == 0:
            path = checkpoint_dir / f"cfr_liberal_iter_{iteration}.pkl"
            data = {
                "iterations": iteration,
    ...
    path = checkpoint_dir / f"cfr_liberal_iter_{NUM_ITERATIONS}.pkl"
    data = {
